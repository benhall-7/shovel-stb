...
impl Stb {
    /// Write the table as CSV to any writer. When `bom` is true a UTF-8 BOM is
    /// prepended so that Excel opens the file with the correct encoding.
    ...
    /// Read a table from CSV. Hashes and groups are recomputed.
    pub fn read_csv<R: Read>(reader: R) -> Result<Self, csv::Error> {
    ...
    pub fn open_csv(path: impl AsRef<std::path::Path>) -> Result<Self, csv::Error> {
